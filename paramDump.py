...
address = uri_helper.uri_from_env(default="radio://0/80/2M/E7E7E7E703")
    ...
    def _connected(self, link_uri):
        ...
        for group in sorted(p_toc.keys()):
            ...
            self._cf.param.add_update_callback(
                group=group, name=None, cb=self._param_callback
            )

        # You can also register a callback for a specific group.name combo
        self._cf.param.add_update_callback(
            group="cpu", name="flash", cb=self._cpu_flash_callback
        )
